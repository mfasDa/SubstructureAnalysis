...
TH1 *extractConstituentSpectrum(THnSparse *dataCharged, const char *observable, double jetptmin, double jetptmx) {
  // set cuts
  auto obsbin = -1;
  if(!strcmp(observable, "Ehc")) obsbin = 4;
  else if(!strcmp(observable, "Enlc")) obsbin = 5;
  else if(!strcmp(observable, "zne")) obsbin = 6;
...
JetRData ExtractTHnSparses(TFile &datafile, const char *trigger, double r) {
  ...
  auto chargedsparse = static_cast<THnSparse *>(histlist->FindObject(Form("hNeutralConstituentsfulljets_R%02d", int(r*10.))));
  return {jetsparse, chargedsparse, r};
}

void extractNeutralConstituentSpectra(const char *datafile, const char *trigger){
  ...
  for(auto r : datasets) {
    ...
    make_frame(Form("Ene_R%02d", int(r.first)), "E_{nlc} (GeV)", "1/N_{jet} dN_{ne}/dE_{nlc} ((GeV)^{-1})", ptframe)->Draw("axis");
    if(row == 0) {
    ...
    for(auto s : GetNormalizedSpectra(r.second, "Enlc")){
    ...
    make_frame(Form("zne_R%02d", int(r.first)), "z_{ne}", "1/N_{jet} dN_{ne}/dz_{ne}", zframe)->Draw("axis");
    auto label = new TPaveText(0.15, 0.82, 0.35, 0.89, "NDC");
    InitWidget<TPaveText>(*label);
    label->AddText(Form("Full jets, R=%0.1f", r.first));
    label->Draw();
    for(auto s  : GetNormalizedSpectra(r.second, "zne")){
      auto spec = s.fSpectrum;
      styles.find(s.fMin)->second.SetStyle(*spec);
      spec->Draw("epsame");
    }
