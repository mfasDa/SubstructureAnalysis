...
std::vector<int> FindDeadFastORs(const TH2 &trgspectra, double threshold = 0) {
  ...
  auto geo = AliEMCALGeometry::GetInstance();   // run2 geometry
  std::cout << "Trigger mapping version: " << geo->GetTriggerMappingVersion() << std::endl;
  for(auto col : ROOT::TSeqI(0, 48)){
    for(auto row : ROOT::TSeqI(0, (geo->GetTriggerMappingVersion() == 2) ? 104 : 60)){
...
void extractDeadFastORsFreq(int triggerlevel = 1, const char *trigger = "INT7", const std::string_view filename = "AnalysisResults.root", int runnumber = 260000){
  ...
  auto geo = AliEMCALGeometry::GetInstanceFromRunNumber(runnumber);   // run2 geometry
