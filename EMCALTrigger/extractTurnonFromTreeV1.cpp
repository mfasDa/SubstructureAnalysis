...
TH1 *extractRawSpectrum(const std::string_view treename){
    const double binning[] = {10., 15., 20., 25., 30., 35., 40., 50., 60., 70, 80, 90., 100., 120., 140, 160., 200.};
    ROOT::RDataFrame dataframe(treename, "AnalysisResults.root");
...
std::map<std::string, int> readNriggers(const std::string_view filename = "AnalysisResults.root"){
...
void extractTurnonFromTreeV1(){
    ...
    for(auto radius : ROOT::TSeqI(2, 6.)){
        ...
        for(auto trg : triggers){
            ...
            filename << "JetSubstructureTree_FullJets_R" << std::setw(2) << std::setfill('0') << radius << "_" << trg;
    ...
    plot->cd(2);
    (new ROOT6tools::TAxisFrame("turnonframeEJ2", "p_{t,jet} (GeV/c)", "Trigger / Min. Bias", 0., 200., 0., 10000.))->Draw("axis");
