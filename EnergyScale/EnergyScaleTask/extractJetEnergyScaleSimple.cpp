...
std::array<TGraphErrors *, 3> getEnergyScaleForRadius(TFile &reader, const std::string_view jettype, double r, const std::string_view sysvar){
  TGraphErrors *mean = new TGraphErrors, *median = new TGraphErrors, *width = new TGraphErrors;
  std::stringstream dirnamebuilder;
  dirnamebuilder << "EnergyScaleResults_" <<  jettype << "_R" << std::setw(2) << std::setfill('0') << int(r*10.) << "_INT7";
  ...
  auto h2d = static_cast<TH2 *>(histlist->FindObject("hJetEnergyScale"));
  int current(0);
  // Make 5 GeV binning
...
void extractJetEnergyScaleSimple(const std::string_view filename = "AnalysisResults.root", const std::string_view jettype = "FullJet", const std::string_view sysvar = ""){
  ...
  outfilename << ".root";
  auto plot = new TCanvas(plotname.str().data(), "Energy scale", 1200,  600);
  plot->Divide(3,1);
  ...
  gPad->SetLeftMargin(0.14);
  gPad->SetRightMargin(0.06);
  auto widthframe = new TH1F("widthframe", "; p_{t,part} (GeV/c); #sigma((p_{t,det} - p_{t,part})/p_{t,part})", 400, 0., 400.);
