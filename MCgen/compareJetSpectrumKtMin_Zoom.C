...
void compareJetSpectrumKtMin_Zoom(const char *filename = "AnalysisResults.root") {
    double xmin = 0., xmax = 100.;
    auto plot = new ROOT6tools::TSavableCanvas("ComparisonJetskTMinZoom", "Comparison jet kt-min (zoom)", 1200, 800);
    plot->Divide(3,2);
    int ipad = 1;
    for(auto R : ROOT::TSeqI(2, 7)) {
        plot->cd(ipad);
        gPad->SetLogy();
        (new ROOT6tools::TAxisFrame(Form("outlierJetR%02d", R), "p_{t,j}", "d#sigma/dp_{t,j} (mb/(GeV/c))", xmin, xmax, 1e-7, 1e2))->Draw();
    ...
    // make binary ratios
    auto ratioplot = new ROOT6tools::TSavableCanvas("RatiosJetskTMinZoom", "Comparison jet kt-min", 1200, 800);
    ratioplot->Divide(3,2);
    ipad = 1;
    auto ratiolegend = new ROOT6tools::TDefaultLegend(0.45, 0.45, 0.89, 0.89);
    for(auto R : ROOT::TSeqI(2, 7)){
        ratioplot->cd(ipad); 
        (new ROOT6tools::TAxisFrame(Form("ratioframeR%02d", R), "p_{t} (GeV/c)", "Ratio k_{t,min}", xmin, xmax, 0., 1.2))->Draw("axis");
