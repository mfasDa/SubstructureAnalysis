...
std::vector<ptbin> readDistsR(TFile &reader, int R) {
    ...
    auto dataZg = project(*static_cast<TH2 *>(histlist->FindObject("hZgPartLevelFine")), R, "Zg"),
         dataRg = project(*static_cast<TH2 *>(histlist->FindObject("hRgPartLevelFine")), R, "Rg"),
         dataThetag = project(*static_cast<TH2 *>(histlist->FindObject("hThetagPartLevelFine")), R, "Thetag"),
         dataNsd = projectNsd(*static_cast<TH2 *>(histlist->FindObject("hNsdPartLevelFine")), R, "Nsd");
...
void extractMCTruePythia(const char *inputfile = "AnalysisResults.root") {
    ...
    std::unique_ptr<TFile> writer(TFile::Open("mcsoftdrop.root", "RECREATE"));
