...
const Float_t phimax = TMath::TwoPi();
const Float_t etamin = -1.;
const Float_t etamax = 1.;

void plot_PWG4HighPtTrackQAHybrid2013_Eta(const char *ContainerInputMerged = "outputAliPWG4HighPtSpectra.root", Int_t cent = 10, Int_t trig = 0, Bool_t bESD = kFALSE, Int_t run = 0, Bool_t bScaleNEvt = kTRUE)
{
  ...
  if (histsGlobAll)
    fPtPhiGlobAll = static_cast<TH2 *>(histsGlobAll->FindObject("fPtEta"));
  auto fPtPhiGlobSt = static_cast<TH2 *>(histsGlobSt->FindObject("fPtEta"));
  auto fPtPhiGlobCnoSPD = static_cast<TH2 *>(histsGlobCnoSPD->FindObject("fPtEta"));
  TH2 *fPtPhiGlobCnoITS(nullptr);
  if (histsGlobCnoITS)
    fPtPhiGlobCnoITS = static_cast<TH2 *>(histsGlobCnoITS->FindObject("fPtEta"));
  if (histsGlobAll)
  {
    fPtPhiGlobAll->SetXTitle("p_{T} [GeV]");
    fPtPhiGlobAll->SetYTitle("#eta");
  }
  fPtPhiGlobSt->SetXTitle("p_{T} [GeV]");
  fPtPhiGlobSt->SetYTitle("#eta");
  fPtPhiGlobCnoSPD->SetXTitle("p_{T} [GeV]");
  fPtPhiGlobCnoSPD->SetYTitle("#eta");
  ...
  c1->SaveAs(Form("PtEtaCent%d.png", cent));
  ...
  auto fPhiGlobSum = static_cast<TH1D *>(fPhiGlobSt->Clone());
  ...
  auto frame2 = gPad->DrawFrame(-1.5, 0., 1.5, fPhiGlobSum->GetBinContent(fPhiGlobSum->GetMaximumBin()) * 2.); //1.5);
  ...
  if (histsGlobCnoITS)
  {
    ...
    fPhiGlobSum2 = static_cast<TH1D *>(fPhiGlobSt->Clone());
  ...
  if (histsGlobAll)
    TH1D *fPtGlobAll = fPtPhiGlobAll->ProjectionX("fPtGlobAll");
...
*/
  ...
  c3->SaveAs(Form("PtEtaCent%d.png", cent));

  std::unique_ptr<TFile> histos(TFile::Open("TrackEtaSpectra.root", "RECREATE"));
