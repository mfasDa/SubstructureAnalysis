...
void ComparisonFullCharged(int reg = 4){
    ...
    auto plot = new ROOT6tools::TSavableCanvas("comparisonFullCharged", "comparison full / charged jets", jetradii.size() * 300, 700);
    ...
    for(auto R : jetradii){
        ...
        specpad.Draw<TH1>(neutralspec, neutralstyle, "charged jets");
        plot->cd(icol+1+jetradii.size());
        RatioPad ratiopad(gPad, R, "charged/full", {0., 350., 0.5, 1.5});
