...
TH1 *getNormalizedJetSpectrum(TFile &reader, double radius, const std::string_view jettype, const std::string_view trigger, int triggercluster){
  ...
  for(const auto &jt : kJetTypes) {
    for(const auto &trg : kTriggers) {
      ...
      for(auto rad : ROOT::TSeqI(2, 6)){
        ...
        auto spec = getNormalizedJetSpectrum(*reader, radius, jt, trg, triggercluster);
