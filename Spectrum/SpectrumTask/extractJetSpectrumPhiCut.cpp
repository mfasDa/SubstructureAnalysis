...
TH1 *makeJetSparseProjection(THnSparse *hsparse, int triggercluster, bool nefcut, double phimin, double phimax) {
  ...
  hsparse->GetAxis(2)->SetRangeUser(phimin, phimax);
...
std::array<TH1 *, 3> getNormalizedJetSpectrum(TFile &reader, double radius, const std::string_view jettype, const std::string_view trigger, int triggercluster, double phimin, double phimax){
  ...
  auto projected = makeJetSparseProjection(jetsparse, triggercluster, jettype == std::string_view("FullJets"), phimin, phimax);
...
void extractJetSpectrumPhiCut(double phimin, double phimax, int triggercluster = 0, const std::string_view inputfile = "AnalysisResults.root"){
  ...
  for(const auto &jt : kJetTypes) {
    for(const auto &trg : kTriggers) {
      ...
      for(auto rad : ROOT::TSeqI(2, 6)){
        ...
        auto spectra = getNormalizedJetSpectrum(*reader, radius, jt, trg, triggercluster, phimin, phimax);
