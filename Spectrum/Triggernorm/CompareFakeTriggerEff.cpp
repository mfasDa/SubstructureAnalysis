...
void CompareFakeTriggerEff(){
    std::vector<Color_t> colors = {kRed, kGreen+2, kBlue, kViolet};
    std::vector<Double_t> jetradii = {0.2, 0.3, 0.4, 0.5};
    auto plot = new ROOT6tools::TSavableCanvas("FakeTrgComp", "Comparison fake trigger efficiency", 800, 600);
    (new ROOT6tools::TAxisFrame("zcomp", "p_t", "Fake / Default", 0., 250, 0.9, 1.05))->Draw("axis");
    ...
    for(auto r : jetradii){
        ...
        auto histdefault = readNormalizedSpectrum(Form("Default/%s", filename.str().data())),
             histzcut = readNormalizedSpectrum(Form("Faked/%s",filename.str().data()));
