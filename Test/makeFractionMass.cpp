...
std::set<ObservableHistJetPtBin> AnalyseTree(const std::string_view filename){
    ...
    for(auto ptbin : ptbins) {
        ...
        auto hist = datahandler.Filter(Form("NEFRec < 1. && ZgMeasured > 0 && PtJetRec > %f && PtJetRec < %f", ptmin, ptmax)).Define("FracMass", [](double mg, double mjet) { return mg/mjet; }, {"MgMeasured", "MassRec" }).Histo1D({Form("FracMdist_%d_%d", int(ptmin), int(ptmax)), "FracM template", nbins, min, max}, "FracMass");
        ObservableHistJetPtBin mybin = {ptmin, ptmax, new TH1F(*hist)};
        mybin.resulthist->SetNameTitle(
            Form("FracMdist_%d_%d", int(ptmin), int(ptmax)), 
            Form("Fraction mass distribution from jets for %.1f GeV/c < p_{t, jet} < %.1f GeV/c", ptmin, ptmax));
...
void makeFractionMass() {
    ...
    TCanvas *plot = new TCanvas("FracMplot", "FracMplot", 1200, 1000);
    plot->Divide(3, 3);
    ...
    for(const auto &trg : TRIGGERS){
        ...
        for(const auto &bin : data) {
            ...
            if(itrg == 0) {
                ...
                hframe->SetXTitle("m_{g} / m_{jet}");
                hframe->SetYTitle("1/N_{jet} dN_{jet}/dm_{frac}");
