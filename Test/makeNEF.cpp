...
std::set<ObservableHistJetPtBin> AnalyseTree(const std::string_view filename){
    ...
    for(auto ptbin : ptbins) {
        ...
        auto hist = datahandler.Filter(Form("NEFRec < 1. && PtJetRec > %f && PtJetRec < %f", ptmin, ptmax)).Histo1D({Form("NEFdist_%d_%d", int(ptmin), int(ptmax)), "NEF template", nbins, min, max}, "NEFRec");
        ObservableHistJetPtBin mybin = {ptmin, ptmax, new TH1F(*hist)};
        mybin.resulthist->SetNameTitle(
            Form("NEFdist_%d_%d", int(ptmin), int(ptmax)), 
            Form("NEF-distribution from jets for %.1f GeV/c < p_{t, jet} < %.1f GeV/c", ptmin, ptmax));
    ...
    for(const auto &trg : TRIGGERS){
        ...
        for(const auto &bin : data) {
            ...
            if(itrg == 0) {
                ...
                hframe->SetXTitle("NEF");
                hframe->SetYTitle("1/N_{jet} dN_{jet}/dNEF");
