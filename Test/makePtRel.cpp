...
std::set<ObservableHistJetPtBin> AnalyseTree(const std::string_view filename){
    ...
    for(auto ptbin : ptbins) {
        ...
        auto hist = datahandler.Filter(Form("NEFRec < 1. && PtJetRec > %f && PtJetRec < %f", ptmin, ptmax)).Define("PtRel", [](double ptg, double ptjet) { return ptg/ptjet; }, {"PtgMeasured", "PtJetRec" }).Histo1D({Form("PtReldist_%d_%d", int(ptmin), int(ptmax)), "PtRel template", nbins, min, max}, "PtRel");
        ObservableHistJetPtBin mybin = {ptmin, ptmax, new TH1F(*hist)};
        mybin.resulthist->SetNameTitle(
            Form("PtReldist_%d_%d", int(ptmin), int(ptmax)), 
            Form("pt,rel-distribution from jets for %.1f GeV/c < p_{t, jet} < %.1f GeV/c", ptmin, ptmax));
...
void makePtRel() {
    ...
    TCanvas *plot = new TCanvas("NEFplot", "NEFplot", 1200, 1000);
    plot->Divide(3, 3);
    ...
    for(const auto &trg : TRIGGERS){
        ...
        filenamebuilder << "JetSubstructureTree_" << trg << ".root";
        ...
        for(const auto &bin : data) {
            ...
            if(itrg == 0) {
                ...
                hframe->SetXTitle("p_{t,g} / p_{t,jet}");
                hframe->SetYTitle("1/N_{jet} dN_{jet}/dp_{t,rel}");
