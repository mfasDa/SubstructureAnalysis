...
void compareEfficiencyPeriodByPeriod(const std::string_view track_type, const std::string_view trigger, const std::string_view acceptance, const std::string_view inputdir){
  ...
  // Draw trending
  auto trendplot = new ROOT6tools::TSavableCanvas(Form("trackTrending_%s_%s_%s", track_type.data(), trigger.data(), acceptance.data()), Form("Track trending %s track %s %s", track_type.data(), trigger.data(), acceptance.data()), 1200, 1000);
  trendplot->Divide(3,2);
  Style trendstyle{kBlack, 20};

  std::set<double> ptref;
  int ipad = 1;
  std::unique_ptr<TFile> writer(TFile::Open(Form("trending_%s_%s_%s.root", track_type.data(), trigger.data(), acceptance.data()), "RECREATE"));
  writer->cd();
