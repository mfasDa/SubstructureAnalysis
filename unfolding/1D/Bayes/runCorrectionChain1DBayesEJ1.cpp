...
TH1 *readSmeared(const std::string_view inputfile, bool weighted, bool downscaleweighted, bool dooutlierrejection){
    auto binning = getJetPtBinningNonLinSmearLargeEJ1();
...
void runCorrectionChain1DBayesEJ1(double radius, const std::string_view indatadir = ""){
    std::string datadir;
    ...
    auto hraw = histcopy(dataspectra.find("EJ1")->second);
    ...
    // read MC
    auto binningdet = getJetPtBinningNonLinSmearLargeEJ1(), 
         binningpart = getJetPtBinningNonLinTrueLargeEJ1();
