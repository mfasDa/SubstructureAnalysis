...
void runCorrectionChain1DBayes_SpectrumTaskSimpleFineLow(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::stringstream outputfile;
    outputfile << "correctedBayes_fine_lowpt";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
