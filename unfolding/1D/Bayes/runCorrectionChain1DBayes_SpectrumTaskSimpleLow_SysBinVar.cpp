...
void runCorrectionChain1DBayes_SpectrumTaskSimpleLow_SysBinVar(const std::string_view datafile, const std::string_view mcfile, const std::string_view binvar, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::stringstream outputfile;
    outputfile << "correctedBayes_lowpt_binvar" << binvar;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    auto binningpart = getJetPtBinningNonLinTrueLargeLow(),
         binningdet = getJetPtBinningNonLinSmearLargeBV(binvar);
