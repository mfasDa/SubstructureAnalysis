...
TH1 *makeCombinedRawSpectrum(const TH1 &mb, const TH1 &ej2, double ej2swap, const TH1 &ej1, double ej1swap, double triggerscale){
    auto combined = histcopy(&mb);
    combined->SetDirectory(nullptr);
    double scalefactor = 1 + triggerscale;
    for(auto b : ROOT::TSeqI(combined->GetXaxis()->FindBin(ej2swap), combined->GetXaxis()->FindBin(ej1swap))) {
        ...
        combined->SetBinContent(b, ej2.GetBinContent(b) * scalefactor);
        combined->SetBinError(b, ej2.GetBinError(b) * scalefactor);
    }
    for(auto b : ROOT::TSeqI(combined->GetXaxis()->FindBin(ej1swap), combined->GetXaxis()->GetNbins()+1)) {
        ...
        combined->SetBinContent(b, ej1.GetBinContent(b) * scalefactor);
        combined->SetBinError(b, ej1.GetBinError(b) * scalefactor);
    }
    return combined;
}


void runCorrectionChain1DBayes_SpectrumTaskSimplePoor_SysTrigger(const std::string_view datafile, const std::string_view mcfile, double triggerscale, const std::string_view sysvar = ""){
    ...
    for(double radius = 0.2; radius <= 0.6; radius += 0.1) {
        ...
        auto hraw = makeCombinedRawSpectrum(*mbrebinned, *ej2rebinned, 50., *ej1rebinned, 100., triggerscale);
