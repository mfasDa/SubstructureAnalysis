...
void runCorrectionChain1DBayes_StatUncertaintyTrigger(double radius, const std::string_view indatadir = ""){
    std::string datadir;
    ...
    std::cout << "[Bayes unfolding] Blow up uncertainties for the triggered range ..." << std::endl;
    for(auto b : ROOT::TSeqI(hraw->GetXaxis()->FindBin(70.5), hraw->GetXaxis()->GetNbins()+1)){
        hraw->SetBinError(b, 10 * hraw->GetBinError(b));
    }
    ...
    std::unique_ptr<TFile> writer(TFile::Open(Form("corrected1DBayesStatUncertainty_R%02d.root", int(radius*10.)), "RECREATE"));
