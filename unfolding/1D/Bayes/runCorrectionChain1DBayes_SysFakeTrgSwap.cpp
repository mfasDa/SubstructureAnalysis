...
TH1 *readSmeared(const std::string_view inputfile, double zcut, bool weighted, bool downscaleweighted, bool dooutlierrejection){
    ...
    if(weighted){
        if(dooutlierrejection){
            auto hist = df.Filter(Form("ZLeadingNeutralRec < %f", zcut)).Filter([](double ptsim, int ptbin) { return !IsOutlierFast(ptsim, ptbin); },{"PtJetSim", "PtHardBin"}).Histo1D({"spectrum", "spectrum", static_cast<int>(binning.size()-1), binning.data()}, "PtJetRec", "PythiaWeight");
            result = histcopy(hist.GetPtr());
        } else {
            auto hist = df.Filter(Form("ZLeadingNeutralRec < %f", zcut)).Histo1D({"spectrum", "spectrum", static_cast<int>(binning.size()-1), binning.data()}, "PtJetRec", "PythiaWeight");
            result = histcopy(hist.GetPtr());
        }
    } else if(downscaleweighted){
        // data - no outlier rejection
        auto hist = df.Filter(Form("ZLeadingNeutralRec < %f", zcut)).Histo1D({"spectrum", "spectrum", static_cast<int>(binning.size()-1), binning.data()}, "PtJetRec", "EventWeight");
        result = histcopy(hist.GetPtr());
    } else {
        if(dooutlierrejection) {
            auto hist = df.Filter(Form("ZLeadingNeutralRec < %f", zcut)).Filter([](double ptsim, int ptbin) { return !IsOutlierFast(ptsim, ptbin); },{"PtJetSim", "PtHardBin"}).Histo1D({"spectrum", "spectrum", static_cast<int>(binning.size()-1), binning.data()}, "PtJetRec");
            result = histcopy(hist.GetPtr());
        } else {
            auto hist = df.Filter(Form("ZLeadingNeutralRec < %f", zcut)).Histo1D({"spectrum", "spectrum", static_cast<int>(binning.size()-1), binning.data()}, "PtJetRec");
            result = histcopy(hist.GetPtr());
        }
    }
...
void runCorrectionChain1DBayes_SysFakeTrgSwap(double radius, double zcut, double ptswap, const std::string_view indatadir = ""){
    std::string datadir;
    ...
    for(const auto &trg : triggers) {
        ...
        auto spec = readSmeared(filename.str(), zcut, true, false, true);
    ...
    for(const auto &trg : triggers) {
        ...
        auto spec = readSmeared(filename.str(), zcut, false, trg == "EJ2", false);
    ...
    auto trgeff = histcopy(mcspectra.find("EJ2")->second);  // Assume EJ2 describes best EJ1
    trgeff->SetName(Form("Efficiency_R%02d_%s", int(radius*10.), "EJ2"));
    trgeff->Divide(trgeff, reference, 1., 1., "b");
    efficiencies.insert({"EJ2",trgeff});
    for(auto &trg : triggers) {
        if(trg == "INT7") continue;
        auto tocorrect = dataspectra.find(trg)->second;
        tocorrect->Divide(trgeff);
    }
    ...
    for(auto b : ROOT::TSeqI(0, hraw->GetNbinsX())){
        if(hraw->GetXaxis()->GetBinCenter(b+1) < ptswap) continue;       // Use data from INT7 trigger
  ...
  
    {
        ...
        TTreeReaderValue<double>  ptrec(mcreader, "PtJetRec"), 
                                  ...
                                  zneutralrec(mcreader, "ZLeadingNeutralRec"),
