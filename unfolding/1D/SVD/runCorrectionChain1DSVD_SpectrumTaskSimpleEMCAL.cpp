...
void runCorrectionChain1DSVD_SpectrumTaskSimpleEMCAL(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    std::stringstream outputfile;
    outputfile << "correctedSVD_EMCAL";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    auto binningpart = getJetPtBinningNonLinTrueEMCAL(),
         binningdet = getJetPtBinningNonLinSmearEMCAL();
