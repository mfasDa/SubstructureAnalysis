...
void runCorrectionChain1DSVD_SpectrumTaskSimpleLow_SysBinVar(const std::string_view datafile, const std::string_view mcfile, const std::string_view binvar, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::stringstream outputfile;
    outputfile << "correctedSVD_lowpt_binvar" << binvar;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
