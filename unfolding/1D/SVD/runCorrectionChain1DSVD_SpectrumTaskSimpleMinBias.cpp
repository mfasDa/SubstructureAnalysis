...
void runCorrectionChain1DSVD_SpectrumTaskSimpleMinBias(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    std::stringstream outputfile;
    outputfile << "correctedSVD_minbias";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    for(double radius = 0.2; radius <= 0.6; radius += 0.1) {
        ...
        auto hraw = mbspectrum.second->Rebin(binningdet.size()-1, "hraw", binningdet.data());
