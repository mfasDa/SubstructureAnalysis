...
void runCorrectionChain1DSVD_SpectrumTaskSimplePoor_SysAngularity(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = "", bool highAngularity, int radiusSel = -1, bool doMT = false){
    ...
    for(auto R : ROOT::TSeqI(2, 7)) {
        ...
        // Numerator must be handled without the angularity splitting and reweighting
        // and therefore to be takend from the original response
        std::unique_ptr<TH1> jeffselected(rawresponseNoReweight->ProjectionY());
        auto jetFindingEff = jeffselected->Rebin(binningpart.size() - 1, Form("hJetfindingEfficiency_R%02d", R), binningpart.data());
        ...
        // Get jet finding efficiency
        // must be taken from the response sample (without angularity splitting)
        std::unique_ptr<TH1> jeffselectedClosure(responseclosureNoReweight->ProjectionY());
        auto jetFindingEffClosure = jeffselected->Rebin(binningpart.size() - 1, Form("hJetfindingEfficiency_R%02d_closure", R), binningpart.data());
