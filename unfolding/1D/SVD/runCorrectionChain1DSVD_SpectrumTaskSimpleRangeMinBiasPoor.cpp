...
void runCorrectionChain1DSVD_SpectrumTaskSimpleRangeMinBiasPoor(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    std::stringstream outputfile;
    outputfile << "correctedSVD_rangeMinBiasPoor";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    auto binningpart = getJetPtBinningNonLinTrueMinBiasPoor(),
         binningdet = getJetPtBinningNonLinSmearMinBiasPoor();
