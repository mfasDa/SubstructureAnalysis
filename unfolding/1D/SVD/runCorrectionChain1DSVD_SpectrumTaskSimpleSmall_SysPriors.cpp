...
void runCorrectionChain1DSVD_SpectrumTaskSimpleSmall_SysPriors(const std::string_view datafile, const std::string_view mcfile, const std::string_view priorsfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::stringstream outputfile;
    outputfile << "correctedSVD_small_priors";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
