...
TH1 *makeCombinedRawSpectrum(const TH1 &mb, const TH1 &ej2, double ej2swap, const TH1 &ej1, double ej1swap){
    ...
    for(auto b : ROOT::TSeqI(combined->GetXaxis()->FindBin(1), combined->GetXaxis()->FindBin(ej2swap))) {
        std::cout << "[" << combined->GetXaxis()->GetBinLowEdge(b) << " - " << combined->GetXaxis()->GetBinUpEdge(b) << "] Using scaled EJ2" << std::endl;
        auto scale = mb.GetBinContent(b)/ej2.GetBinContent(b);
        combined->SetBinContent(b, ej2.GetBinContent(b)*scale);
        combined->SetBinError(b, ej2.GetBinError(b)*scale);
    }
...
void runCorrectionChain1DSVD_SpectrumTaskSimpleTriggerLow(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar){
    ROOT::EnableThreadSafety();
    std::stringstream outputfile;
    outputfile << "correctedSVD_triggerLow";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
