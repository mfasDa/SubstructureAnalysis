...
void runCorrectionChain1DSVD_SpectrumTaskSimpleUltraOutlier(const std::string_view datafile, const std::string_view mcfile, int ultraoption, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    std::function<std::vector<double> ()> binhandlerTrue, binhandlerSmear;
    if(ultraoption == 240) {
        binhandlerTrue = getJetPtBinningNonLinTrueUltra240;
        binhandlerSmear = getJetPtBinningNonLinSmearUltra240;
    } else if(ultraoption == 300) {
        binhandlerTrue = getJetPtBinningNonLinTrueUltra300;
        binhandlerSmear = getJetPtBinningNonLinSmearUltra300;
    } else {
        std::cout << "Ultra option not available" << std::endl;
        return;
    }
    std::stringstream outputfile;
    outputfile << "correctedSVD_ultra" << ultraoption;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    auto centnotrdcorrection = getCENTNOTRDCorrection(*datareader, "tc200");
    double crosssection = 57.8;
    for(double radius = 0.2; radius <= 0.6; radius += 0.1) {
        std::cout << "Doing jet radius " << radius << std::endl;
        auto mbspectrum = getSpectrumAndNorm(*datareader, radius, "INT7", "ANY", "tc200"),
             ej1spectrum = getSpectrumAndNorm(*datareader, radius, "EJ1", "CENTNOTRD", "tc200"),
             ej2spectrum = getSpectrumAndNorm(*datareader, radius, "EJ2", "CENT", "tc200");
