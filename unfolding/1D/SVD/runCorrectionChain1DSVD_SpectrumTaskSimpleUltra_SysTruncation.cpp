...
void runCorrectionChain1DSVD_SpectrumTaskSimpleUltra_SysTruncation(const std::string_view datafile, const std::string_view mcfile, int ultraoption, const std::string_view truncvar, const std::string_view sysvar = ""){
    ...
    if(ultraoption == 240) {
        ...
        binhandlerSmear = getJetPtBinningNonLinSmearUltra240Trunc;
    } else if(ultraoption == 300) {
        binhandlerTrue = getJetPtBinningNonLinTrueUltra300;
        binhandlerSmear = getJetPtBinningNonLinSmearUltra300Trunc;
    } else {
        std::cout << "Ultra option not available" << std::endl;
        return;
    }
    std::stringstream outputfile;
    outputfile << "correctedSVD_ultra" << ultraoption << "_truncation" << truncvar;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
