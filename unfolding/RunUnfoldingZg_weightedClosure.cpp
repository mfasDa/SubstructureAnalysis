...
void RunUnfoldingZg_weightedClosure(const std::string_view filedata, const std::string_view filemc, double nefcut = 0.02, double fracSmearClosure = 0.5){
  ...
  auto dataextractor = [nefcut](const std::string_view filedata, double ptsmearmin, double ptsmearmax, TH2D *hraw) {
    ...
    //auto datahist = recframe.Filter(Form("NEFRec < 1-%f && PtJetRec > %f && PtJetRec < %f", nefcut, ptsmearmin, ptsmearmax)).Histo2D(*hraw, "ZgMeasured", "PtJetRec");
  ...
  auto reweighter = [](const TH2 *hdata, TH2 *hsmeared, TH2 *hsmearedclosure) {
      ...
      for(auto bpt : ROOT::TSeqI(0, hdata->GetYaxis()->GetNbins())){
            ...
            for(auto bzg : ROOT::TSeqI(0, hsmeared->GetXaxis()->GetNbins())){
                hsmeared->SetBinContent(bzg+1, bpt+1, hsmeared->GetBinContent(bzg+1, bpt+1) * weightsmear->GetBinContent(bzg+1));
                hsmeared->SetBinError(bzg+1, bpt+1, hsmeared->GetBinError(bzg+1, bpt+1) * weightsmear->GetBinContent(bzg+1));
                hsmearedclosure->SetBinContent(bzg+1, bpt+1, hsmearedclosure->GetBinContent(bzg+1, bpt+1) * weightsmearclosure->GetBinContent(bzg+1));
                hsmearedclosure->SetBinError(bzg+1, bpt+1, hsmearedclosure->GetBinError(bzg+1, bpt+1) * weightsmearclosure->GetBinContent(bzg+1));
            }
        }
  };
