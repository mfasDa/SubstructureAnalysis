...
void ComparisonFoldRaw_8TeV(const std::string_view inputfile, string outputdir, string filetype = "png", int plotptmax = 250.){
    ...
    for(auto r : jetradii){
        ...
        SetStyleHistoTH2ForGraphs(dummyHistUpper, "#it{p}_{T} (GeV/#it{c})", "1/N_{ev} dN/dp_{T} ((GeV/c)^{-1})", 0.85*textsizeLabelsWidth, textsizeLabelsWidth,
        ...
        auto rawspectrum = data.GetJetSpectrum(r, "hraw");
        rawspectrum->SetMarkerSize(2);
        specpad.Draw<TH1>(rawspectrum, rawstyle, "raw", "p");
        ...
        SetStyleHistoTH2ForGraphs(dummyHistLower, "#it{p}_{T} (GeV/#it{c})", "Folded/Raw", 0.85*textsizeLabelsComp, textsizeLabelsComp, 0.85*textsizeLabelsComp,
        ...
        for(auto ireg : ROOT::TSeqI(regmin,regmax+1)){
            auto backfolded = data.GetJetSpectrum(r, Form("backfolded_reg%d", ireg));
            backfolded->Scale(1., "width");
            backfolded->SetMarkerSize(2);
        ...
        for(auto ireg : ROOT::TSeqI(regmin,regmax+1)){
            auto backfolded = data.GetJetSpectrum(r, Form("backfolded_reg%d", ireg)); // Already scaled by width from previous loop
            Style varstyle{colors[ireg-1], markers[ireg-1]};
            auto ratioraw = new Ratio(backfolded, rawspectrum);
            ratioraw->SetMarkerSize(2);
            ratiopad_ind.Draw<Ratio>(ratioraw, varstyle, Form("reg=%d", ireg));
        }
