...
TH1 *getRegularizationHist(TFile &reader, int reg){
  ...
  if(reader.GetListOfKeys()->FindObject(regname.data())){
    ...
    std::string histname = Form("backfolded_reg%d", reg);
...
TH1 *getiIterationHist(TFile &reader, int reg){
  ...
  if(reader.GetListOfKeys()->FindObject(regname.data())){
    reader.cd(regname.data());
    std::string histname = Form("backfolded_iter%d", reg);
    result = static_cast<TH1 *>(gDirectory->Get(histname.data()));
    if(result){
      result->SetDirectory(nullptr);
      normalizeBinWidth(result);
...
TH1 *getRaw(const std::string_view inputfile){
  std::unique_ptr<TFile> reader(TFile::Open(inputfile.data(), "READ"));
  auto hist = static_cast<TH1 *>(reader->Get("hraw"));
...
void ComparisonFoldRaw(const std::string_view filename){
  ...
  auto plot = new ROOT6tools::TSavableCanvas(Form("comparisonFoldRaw%s_%s", (isSVD ? "Svd" : "Bayes"), tag.data()), "Conparison regularization SVD", 1200, 600);
  plot->Divide(2,1);
  
  plot->cd(1);
  gPad->SetLogy();
  (new ROOT6tools::TAxisFrame("specframe", "p_{t} (GeV/c)", "dN/dp_{t} ((GeV/c)^{-1})", 0., 120., 1e-2, 1e8))->Draw("axis");
  ...
  plot->cd(2);
  (new ROOT6tools::TAxisFrame("ratframe", "p_{t} (GeV/c)", "folded/raw", 0., 120., 0., 2.))->Draw("axis");
