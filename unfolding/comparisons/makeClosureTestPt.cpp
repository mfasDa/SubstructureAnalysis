...
void makeClosureTestPt(const std::string_view inputfile){
  ...
  auto compplot = new ROOT6tools::TSavableCanvas(Form("MCClosurePtComp_%s_R%02d_%s_%s", conf.fJetType.data(), int(conf.fR * 10.), conf.fTrigger.data(), conf.fObservable.data()),
  ...
  // Also draw ratios
  auto ratioplot = new ROOT6tools::TSavableCanvas(Form("MCClosureRatioUnfoldedTruePt_%s_R%02d_%s_%s", conf.fJetType.data(), int(conf.fR * 10.), conf.fTrigger.data(), conf.fObservable.data()),
