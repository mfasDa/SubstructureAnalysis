...
void makePlotTrigger(std::string_view filename, std::string_view trigger, double radius){
    std::set<RangedObject> histograms;
    {
        ...
        for(auto h : TRangeDynCast<TKey>(filereader->GetListOfKeys())){
            ...
            if(histname.find("corrected_mass_") != std::string::npos){
    ...
    auto axis = new TH1F(Form("massaxis_%s_R%02d", trigger.data(), int(radius * 10.)), "; M (GeV/c^{2}); 1/N_{jet} dN/dM ((GeV/c^{2})^{-1})", 100, 0., 40.);
    axis->SetStats(false);
...
void makeComparisonFullyCorrectedJetMass(){
    ...
    for(auto radius : jetradii){
        ...
        for(auto trg : triggers){
            ...
            makePlotTrigger(Form("Unfolded_JetMass_R%02d_%s/corrected_mass_R%02d_%s.root", int(radius * 10.), trg.data(), int(radius * 10.), trg.data()), trg, radius);
            itrg++;
        }
        irad++;
    }
