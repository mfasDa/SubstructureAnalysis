...
std::map<double, TH1 *> readRawSpectra(const std::string_view filename, int ultrarange){
    ...
    if(ultrarange == 240) {
        ptmaxreport = 241.;
        binningtmp = getJetPtBinningNonLinSmearUltra240();
    } else if (ultrarange == 300){
        ptmaxreport = 301.;
        binningtmp = getJetPtBinningNonLinSmearUltra300();
    }
...
void makeComparisonRawTruncationConstituents(int ultrarange) {
    ...
    double maxy;
    if(ultrarange == 240) maxy = 300;
    else if (ultrarange == 300) maxy = 350;
    Style refstyle{kBlack, 20};

    auto plot = new ROOT6tools::TSavableCanvas(Form("comparisonTruncationClusterTrackRaw_unfolding%d", ultrarange), Form("Comparison unfolded spectra with different cluster / track cuts"), 300 * reference.size(), 700);
