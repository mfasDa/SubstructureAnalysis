...
void makeComparisonUnfoldedRestrictedHigh(const std::string_view sysvar = "notc"){
    ...
    auto dataUltra = JetSpectrumReader(Form("correctedSVD_ultra240_%s.root", sysvar.data()), spectra), 
         dataReg = JetSpectrumReader(Form("correctedSVD_ultra300_%s.root", sysvar.data()), spectra);
    auto jetradii = dataUltra.GetJetSpectra().GetJetRadii();
    Restrictor reported(20., 320.);

    auto plot = new ROOT6tools::TSavableCanvas(Form("comparisonUnfoldedRangesHigh%s", sysvar.data()), "Comparison raw spectra in identical range", 300 * jetradii.size(), 700);
    ...
    for(auto r : jetradii){
        ...
        specpad.Frame(Form("specframeR%02d", int(r * 10.)), "p_{t} (GeV/c)", "d#sigma/dp_{t}d#eta (mb/(GeV/c))", 0., 350, 1e-9, 1e-2);
        ...
        specpad.Draw<TH1>(specreg, regstyle, "10 GeV/c < p_{t,j,det} < 240 GeV/c");
        specpad.Draw<TH1>(specultra, ultrastyle, "10 GeV/c < p_{t,j,det} < 320 GeV/c");

        plot->cd(icol+jetradii.size()+1);
        GraphicsPad ratiopad(gPad);
        ratiopad.Margins(0.17, 0.04, -1., 0.02);
        ratiopad.Frame(Form("specframeR%02d", int(r * 10.)), "p_{t} (GeV/c)", "large range /vormal range", 0., 350, 0.8, 1.2);
