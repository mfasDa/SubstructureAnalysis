...
std::vector<variation> getVariations(int ultrarange) {
    ...
    if(ultrarange == 300)result.push_back({300, {kRed, 24}});
    return result;
}

std::string makeLegEntry(int tccut, std::string type) {
    std::stringstream legentry;
    legentry << "p_{t}^{t} < ";
    if(type == "track") legentry << tccut;
    else legentry << 200;
    legentry << " GeV/c, p_{t}^{c} < ";
    if(type == "cluster") legentry << tccut;
    else legentry << 200;
    legentry << " GeV/c";
    return legentry.str();
}

void makeComparisonUnfoldedTruncationConstituentsSeparate(int ultrarange, std::string type) {
    ...
    for(auto v : getVariations(ultrarange)) {
        ...
        filename << "correctedSVD_ultra_" << ultrarange;
        if(type == tracks) filename << "t" << v.fTC << "c200"
        else filename << "t200c" << v.fTC;
        filename << ".root";
    ...
    auto plot = new ROOT6tools::TSavableCanvas(Form("comparisonTruncationClusterTrack_unfolding%d_var%s", ultrarange, type.data()), Form("Comparison unfolded spectra with different cluster / track cuts"), 300 * jetradii.size(), 700);
    ...
    for(double r = 0.2; r <= 0.6; r += 0.1) {
        ...
        if(!icol) {
            ...
            specpad.Legend(0.35, 0.5, 0.89, 0.89);
        }
        ...
        for(auto v : variations) {
            ...
            specpad.Draw<TH1>(varspec, v.first.Style, makeLegEntry(v.first.fTCcut, type));
