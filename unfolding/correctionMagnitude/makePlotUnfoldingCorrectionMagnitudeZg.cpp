...
std::map<int, TH2 *> extractUnfoldingRatios(TFile &reader) {
    ...
    for(auto b : ROOT::TSeqI(1, 35)) {
        ...
        auto ratio_unfolded_raw  = static_cast<TH2 *>(unfolded_reduced->Clone(Form("ratio_unfolded_raw_iter%d", b)));
...
void makePlotUnfoldingCorrectionZg(std::string_view filename){
    ...
    auto ratiodata = extractUnfoldingRatios(*reader);
    ...
    for(auto b : ROOT::TSeqI(ratiodata[1]->GetYaxis()->FindBin(ptrange.fMin+0.5), ratiodata[1]->GetYaxis()->FindBin(ptrange.fMax-0.5) + 1)){
        ...
        auto axis = new TH1F(Form("ratioaxis%d",pad), "; z_{g}; unfolded/raw", 100, 0., 0.5);
        axis->SetDirectory(nullptr);
        axis->SetStats(false);
        axis->GetYaxis()->SetRangeUser(0., 10.0);
        axis->Draw("axis");
