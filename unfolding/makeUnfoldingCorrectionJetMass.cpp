...
std::vector<PtBin> ExtractUnfoldedBins(const TH2 *data) {
    ...
    for(auto b : ROOT::TSeqI(1, data->GetYaxis()->GetNbins()+1)){
        ...
        slice->SetName(Form("mass_slice_%d_%d", int(data->GetYaxis()->GetBinLowEdge(b)), int(data->GetYaxis()->GetBinUpEdge(b))));
...
void makeUnfoldingCorrectionJetMass(std::string_view unfoldedfile, int iteration = 4){
    // Steps:
    ...
    for(auto s : unfoldedbins){ 
        ...
        auto correctedbin = static_cast<TH1 *>(s.fData->Clone(Form("corrected_mass_%d_%d", (int)s.fPtMin, (int)s.fPtMax)));
    ...
    outname <<  "corrected_mass_R" << std::setw(2) << std::setfill('0') << int(radius * 10.) << "_" << trigger << ".root";
