...
void plotStatRelRaw(const std::string_view inputfile){
    std::vector<std::string> spectra = {"hraw"};
    JetSpectrumReader reader(inputfile, spectra);
    
    auto plot = new ROOT6tools::TSavableCanvas("relStatErrorRaw", "Relative statistical error of the raw spectrum", 1200, 800);
    plot->Divide(3,2);
    ...
    for(auto r : reader.GetJetSpectra().GetJetRadii()){
        ...
        errorpad.Frame(Form("errorpad%s", rstring.data()), "p_{t} (GeV/c)", "rel. stat. error", 0., 240., 0., 0.1);
