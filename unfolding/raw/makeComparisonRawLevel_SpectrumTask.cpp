...
void DrawSpectra(const std::map<std::string, TH1 *> &spectra, Double_t r, Bool_t doleg, Bool_t rebinned) {
    ...
    (new ROOT6tools::TNDCLabel(0.15, 0.15, 0.35, 0.22, Form("R=%.1f", r)));
...
void makeComparisonRawLevel_SpectrumTask(const std::string_view corrfile, const std::string sysvar) {
    ...
    std::map<std::string, std::string> clusters = {{"INT7", "ANY"}, {"EJ1", "CENTNOTRD"}, {"EJ2", "CENT"}};
    ...
    for(auto r : ROOT::TSeqI(2,7)){
        ...
        plotfine->cd(icol);
        DrawSpectra(triggersFine, double(r)/10., icol == 1, false);
        plotrebinned->cd(icol);
        DrawSpectra(triggersRebinned, double(r)/10., icol == 1, true);

        plotfine->cd(icol+5);
        DrawRatios(triggersFine, double(r)/10., false);
        plotrebinned->cd(icol+5);
        DrawRatios(triggersRebinned, double(r)/10., true);
        icol++;
    }
    plotfine->cd();
    plotfine->Update();
