...
std::pair<TGraphErrors *, TGraphErrors *> readfile(const std::string_view filename) {
    ...
    gROOT->cd();
    return {static_cast<TGraphErrors *>(reader->Get("mean")), static_cast<TGraphErrors *>(reader->Get("sigma"))};
}

void makeEnergyScaleComparison(){
    ...
    auto plot = new ROOT6tools::TSavableCanvas("jeteneryscalefull", "Jet energy scale full jets", 1200, 800);
    plot->Divide(2,1);
    plot->cd(1);
