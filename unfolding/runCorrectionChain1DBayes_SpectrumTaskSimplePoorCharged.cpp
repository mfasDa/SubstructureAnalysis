...
class UnfoldingRunner {
    ...
    private:
        unfoldingResults runUnfolding(const UnfoldingConfiguration &config){
            ...
            const double kSizeEta = 1.8;
            auto acceptance = (kSizeEta - 2 * config.fRadius);
...
TH2 *getResponseMatrix(TFile &reader, double R, const std::string_view sysvar, int closurestatus = 0) {
    ...
    dirnamebuilder << "EnergyScaleResults_ChargedJet_R" << std::setw(2) << std::setfill('0') << int(R*10.) << "_INT7";
...
void runCorrectionChain1DBayes_SpectrumTaskSimplePoorCharged(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ...
    for(double radius = 0.2; radius <= 0.6; radius += 0.1) {
        ...
        auto hraw = mbspectrum.second->Rebin(binningdet.size()-1, "mbrebinned", binningdet.data());
