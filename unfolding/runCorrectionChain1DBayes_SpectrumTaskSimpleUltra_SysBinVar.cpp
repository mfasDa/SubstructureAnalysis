...
void runCorrectionChain1DBayes_SpectrumTaskSimpleUltra_SysBinVar(const std::string_view datafile, const std::string_view mcfile, int ultraoption, const std::string_view binvar, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::function<std::vector<double> ()> binhandlerTrue;
    std::function<std::vector<double> (const std::string_view)> binhandlerSmear;
    if(ultraoption == 240) {
        binhandlerTrue = getJetPtBinningNonLinTrueUltra240;
        binhandlerSmear = getJetPtBinningNonLinSmearUltra240BV;
    } else if(ultraoption == 300) {
        binhandlerTrue = getJetPtBinningNonLinTrueUltra300;
        binhandlerSmear = getJetPtBinningNonLinSmearUltra300BV;
    } else {
        std::cout << "Ultra option not available" << std::endl;
        return;
    }
    std::stringstream outputfile;
    outputfile << "correctedBayes_ultra" << ultraoption << "_binvar" << binvar;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    auto binningpart = binhandlerTrue(),
         binningdet = binhandlerSmear(binvar);
