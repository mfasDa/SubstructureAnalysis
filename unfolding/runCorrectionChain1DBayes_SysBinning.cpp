...
#include "binnings/binningPt1D_bv.C"

std::vector<std::string> triggers = {"INT7", "EJ1", "EJ2"};

TH1 *readSmeared(const std::string_view inputfile, const std::string_view option, bool weighted, bool downscaleweighted, bool dooutlierrejection){
    auto binning = getJetPtBinningNonLinSmearLarge(option);
...
std::vector<TH1 *> extractCENTNOTRDCorrection(const std::string_view filename, const std::string_view option){
    auto binning = getJetPtBinningNonLinSmearLarge(option);
...
void runCorrectionChain1DBayes_SysBinning(double radius, const std::string_view option, const std::string_view indatadir = ""){
    std::string datadir;
    ...
    auto centnotrdCorrection = extractCENTNOTRDCorrection(Form("%s/data/merged_17/JetSubstructureTree_FullJets_R%02d_EJ1.root", datadir.data(), int(radius*10.)), option);
    ...
    for(const auto &trg : triggers) {
        ...
        auto spec = readSmeared(filename.str(), option, true, false, true);
    ...
    for(const auto &trg : triggers) {
        ...
        auto spec = readSmeared(filename.str(), option, false, trg == "EJ2", false);
    ...
    // read MC
    auto binningdet = getJetPtBinningNonLinSmearLarge(option), 
