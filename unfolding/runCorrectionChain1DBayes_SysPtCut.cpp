...
void runCorrectionChain1DBayes_SysPtCut(double radius, double ptcut, const std::string_view indatadir = ""){
    std::string datadir;
  ...
  
    {
        ...
        filemc << datadir << "/mc/merged_calo/JetSubstructureTree_FullJets_R" << std::setw(2) << std::setfill('0') << int(radius*10.) << "_INT7_pt" << int(ptcut) <<  "_merged.root";
    ...
    std::unique_ptr<TFile> writer(TFile::Open(Form("corrected1DBayes_R%02d_pt%d.root", int(radius*10.), int(ptcut)), "RECREATE"));
