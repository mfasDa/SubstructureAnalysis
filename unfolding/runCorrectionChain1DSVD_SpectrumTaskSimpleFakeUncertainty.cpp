...
TH1 *makeCombinedRawSpectrum(const TH1 &mb, const TH1 &ej2, double ej2swap, const TH1 &ej1, double ej1swap){
    ...
    for(auto b : ROOT::TSeqI(1, combined->GetXaxis()->FindBin(ej2swap))){
        combined->SetBinError(b, 0.05 * combined->GetBinContent(b));
    }
...
void runCorrectionChain1DSVD_SpectrumTaskSimpleFakeUncertainty(const std::string_view datafile, const std::string_view mcfile, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    std::stringstream outputfile;
    outputfile << "correctedSVD_fakeuncertainty";
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
    ...
    for(double radius = 0.2; radius <= 0.6; radius += 0.1) {
        ...
        auto hraw = makeCombinedRawSpectrum(*mbrebinned, *ej2rebinned, 70., *ej1rebinned, 100.);
