...
void runCorrectionChain1DSVD_SpectrumTaskSimpleLow_SysTruncation(const std::string_view datafile, const std::string_view mcfile, const std::string_view truncvar, const std::string_view sysvar = ""){
    ROOT::EnableThreadSafety();
    int NTHREAD=2;
    std::stringstream outputfile;
    outputfile << "correctedSVD_lowpt_truncation" << truncvar;
    if(sysvar.length()) {
        outputfile << "_" << sysvar;
    }
    outputfile << ".root";
