...
void plotStatsZgIdeal(const std::string_view trigger){
  std::vector<double> ptbinning = getPtBinningFine(trigger), zgbinning = getZgBinningFine();
  print(ptbinning);
  print(zgbinning);
  std::stringstream canvasname, canvastitle;
  canvasname << "StatZgIdeal" << trigger;
  canvastitle << "Statistics Zg for trigger " << trigger << " with ideal binning";
  ...
  for(auto r : ROOT::TSeqI(2, 6)) {
    ...
    histname << "CountsZgPtIdealR" << std::setw(2) << std::setfill('0') << r;
