...
void plotStatsZgLowGranularity(const std::string_view trigger){
  std::vector<double> ptbinning = getPtBinningRealistic(trigger), zgbinning = getZgBinningCoarse();
  print(ptbinning);
  print(zgbinning);
  std::stringstream canvasname, canvastitle;
  canvasname << "StatZgLowGranularity" << trigger;
  canvastitle << "Statistics Zg for trigger " << trigger << " with low granularity binning";
  ...
  for(auto r : ROOT::TSeqI(2, 6)) {
    ...
    histname << "CountsZgPtLowGranularityR" << std::setw(2) << std::setfill('0') << r;
