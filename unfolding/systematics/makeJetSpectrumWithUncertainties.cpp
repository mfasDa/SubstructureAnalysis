...
void makeJetSpectrumWithUncertainties(const std::string_view corr, const std::string_view shape){
    ...
    for(double r = 0.2; r <= 0.6; r+=0.1) {
        ...
        auto frame = new ROOT6tools::TAxisFrame(Form("frameR%02d", int(r*10.)), "p_{t} (GeV/c)", "d#sigma/(dp_{t}dy (mb/(GeV/c))", 0, 350, 1e-9, 1);
        frame->Draw("axis");
        ...
        auto spec = reported(static_cast<TH1*>(gDirectory->Get(Form("Specdefault_%s", rstring.data())))),
