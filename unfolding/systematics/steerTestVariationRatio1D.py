...
class VariationGroup:
    ...
    def setjetradiiden(self, jetradii):
        self.__jetradiiden = jetradii

    def setjetradiusnum(self, jetradius):
        self.__radiusnum = jetradius
    ...
    def runcomp(self):
        ...
        for var in self.__variations:
            ...
            rootfiledefaultNum = "corrected1D%s_R%02d.root" %(self.__defaultsettings.getunfoldingmethod(), self.__radiusnum)
            rootfilevarNum = "corrected1D%s_R%02d.root" %(var.getunfoldingmethod(), self.__radiusnum)
            defaultfileNum = os.path.join(self.__defaultsettings.getdatalocation(), rootfiledefaultNum)
            varfileNum = os.path.join(datadir, rootfilevarNum)
            for r in self.__jetradiiden:
                logging.info("Running jet Radius: %.1f", float(r)/10.)
                rootfiledefaultDen = "corrected1D%s_R%02d.root" %(self.__defaultsettings.getunfoldingmethod(), r)
                rootfilevarDen = "corrected1D%s_R%02d.root" %(var.getunfoldingmethod(), r)
                defaultfileDen = os.path.join(self.__defaultsettings.getdatalocation(), rootfiledefaultDen)
                varfileDen = os.path.join(datadir, rootfilevarDen) 
                varname = "%s_%s" %(self.__name, var.getname())
                vartitle = "%s, %s" %(self.__title, var.gettitle())
                logging.info("default file numerator: %s, default file denominator: %s variation file numerator: %s, variation file denominator: %s, methods: %s / %s regularizations: %d / %d", defaultfileNum, defaultfileDen, varfileNum, varfileDen, self.__defaultsettings.getunfoldingmethod(), var.getunfoldingmethod(), self.__defaultsettings.getregularization(), var.getregularization())
                cmd="root -l -b -q \'%s(\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", %d, %d)\'" %(os.path.join(self.__repo, "testVariationRatio1D.cpp"), varname, vartitle, defaultfileNum, defaultfileDen, varfileNum, varfileDen, self.__defaultsettings.getregularization(), var.getregularization())
...
def sortPlots(jetradii):
    ...
    for rad in jetradii:
        for form in formats:
            resultdir = os.path.join(basedir, "RatioR02R%02d" %rad, form)
            if not os.path.exists(resultdir):
                os.makedirs(resultdir, 0755)
            tag = "R02R%02d.%s" %(rad, form)
...
if __name__ == "__main__":
    ...
    for prod in producers_unfolding:
        ...
        group.setjetradiiden(jetradii)
        group.setrepo(repo)
        group.setjetradiusnum(2)
